...
if Execute_code : 
    ...
    for filename in os.listdir(csv_directory):
        if filename.endswith('.csv'):
            ...
            with open(file_path, mode='r', newline='') as csvfile:
                ...
                if len(rows) > 1 and len(rows[1]) > 4:  # Ensure there is a second row and fourth column
                    run_time = rows[1][3]  # Get the value from the second row and fourth column
                    Remaining_noise_budget = rows[1][4]
                    try: 
                        run_time = float(run_time)/1000
                        run_time = format(run_time, ".3f") 
                        run_times[benchmark_name] = run_time
                        ####################################
                        Remaining_noise_budgets[benchmark_name]=Remaining_noise_budget
                    except ValueError:                      
                        run_times[benchmark_name] = run_time
                        Remaining_noise_budgets[benchmark_name]=Remaining_noise_budget
                else :  
    ...
    with open('benchmarks_evaluation.csv', mode='w', newline='') as outfile:
        ...
        print(f"\n===> Obtained remaining noise budgets: {Remaining_noise_budgets}\n")
        for row in rows:
            benchmark_name = row[0]
            if benchmark_name in run_times:
                row.append(run_times[benchmark_name])
                row.append(Remaining_noise_budgets[benchmark_name])
            else:
                row.append('')
                row.append('')
            writer.writerow(row)
